#ifndef GROWTHDAQ_FITSFILE_HH_
#define GROWTHDAQ_FITSFILE_HH_
...
class FitsUtility {
 ...
 public:
  static std::string convertSIB2StyleDataTypeNameToCfitsioStyleTFORM(std::string dataTypeName) {
    dataTypeName = stringutil::toLowerCase(dataTypeName);
    if (stringutil::contain(dataTypeName, "byte")) {
      if (stringutil::contain(dataTypeName, "unsigned")) {
        return "B";
      } else {
        return "S";
      }
    } else if (stringutil::contain(dataTypeName, "short")) {
      if (stringutil::contain(dataTypeName, "unsigned")) {
        return "U";
      } else {
        return "I";
      }
    } else if (stringutil::contain(dataTypeName, "int")) {
      if (stringutil::contain(dataTypeName, "unsigned")) {
        return "V";
      } else {
        return "J";
      }
    } else if (stringutil::contain(dataTypeName, "long")) {
      return "K";
    } else if (stringutil::contain(dataTypeName, "float")) {
      return "E";
    } else if (stringutil::contain(dataTypeName, "double")) {
      return "D";
    } else if (stringutil::contain(dataTypeName, "x") ||
               stringutil::contain(dataTypeName, "bits") ||
               stringutil::contain(dataTypeName, "bit")) {
      std::string dataTypeWidth = stringutil::replace(dataTypeName, "x", "");
      dataTypeWidth = stringutil::replace(dataTypeWidth, "bits", "");
      dataTypeWidth = stringutil::replace(dataTypeWidth, "bit", "");
      dataTypeWidth = stringutil::replace(dataTypeWidth, " ", "");
      return dataTypeWidth + "X";
    } else {
      using namespace std;
      cerr << "CxxUtilities::FitsUtility::convertSIB2StyleDataTypeNameToCfitsioStyleTFORM(): Unrecognizable data type "
              "name "
  ...
  static void getTZEROAndTSCALForSIB2StyleDataTypeName(std::string dataTypeName, double& resultingTZERO,
    ...
    dataTypeName = stringutil::toLowerCase(dataTypeName);
    if (stringutil::contain(dataTypeName, "float") || stringutil::contain(dataTypeName, "double") ||
        stringutil::contain(dataTypeName, "x") || stringutil::contain(dataTypeName, "bit")) {
      // nothing to do
      return;
    }
    if (stringutil::contain(dataTypeName, "unsigned")) {  // unsigned
      switch (type) {
        case 'B':  // unsiged byte
          resultingTSCAL = 1;
          resultingTZERO = 0;
      ...
      switch (type) {
        ...
        default:
          using namespace std;
          cerr
              << "CxxUtilities::FitsUtility::getTZEROAndTSCALForSIB2StyleDataTypeName(): Unrecognizable data type name "
              << dataTypeName << " provided." << endl;
          break;
      }
    }
...
class FitsColumnDefinition {
 ...
 public:
  FitsColumnDefinition(std::string columnName, std::string columnDataType, std::string comment = "") {
    ...
    if (stringutil::containsNumber(cfitsioStyleTFORM)) {
 ...
 public:
  bool isUnitSet() { return isUnitSet_; }
 ...
 public:
  bool isDisplayFormatSet() { return isDisplayFormatSet_; }
};
...
class FitsExtensionDefinition {
 ...
 public:
  FitsExtensionDefinition(std::string extensionName) { this->extensionName = extensionName; }
 ...
 public:
  void push(const FitsColumnDefinition& columnDefinition) { this->columnDefinitions.push_back(columnDefinition); }

 public:
  FitsColumnDefinition& at(size_t index) { return this->columnDefinitions.at(index); }

 public:
  size_t nColumns() { return this->columnDefinitions.size(); }

 public:
  size_t size() { return this->columnDefinitions.size(); }

 public:
  static FitsExtensionDefinition* constructFromListOfColumnNameDataTypeAndComment(
      std::string extensionName, std::string listOfColumnNamesAndDataTypeNames, std::string delimiter = " ") {
    FitsExtensionDefinition* extensionDefinition = new FitsExtensionDefinition(extensionName);
    auto lines = stringutil::splitIntoLines(listOfColumnNamesAndDataTypeNames);
    for (auto& line : lines) {
      std::vector<std::string> array = stringutil::split(line, delimiter);
      if (array.size() < 2) {
        using namespace std;
 ...
 public:
  void setExtensionName(const std::string& extensionName) { this->extensionName = extensionName; }

 private:
  std::vector<std::string> columnNames;
  std::map<std::string, size_t> columnNameAndColumnIndexMap;
  std::vector<std::tuple<std::string /*column name*/, int /*column index*/, int /*data type number*/>>
      columnNameAndColumnIndexVector;
 ...
 public:
  std::string toString() {
    ...
    size_t i = 0;
    for (auto& e : this->columnDefinitions) {
      ss << left << setw(3) << i << " TTYPE=" << setw(32) << e.TTYPE << " TFORM=" << setw(4) << e.TFORM
         << " TUNIT=" << setw(5) << e.TUNIT << " TZERO=" << setw(12) << e.TZERO << " TSCAL=" << setw(12) << e.TSCAL
