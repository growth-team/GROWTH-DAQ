...
class ConsumerManager {
 ...
 public:
  ...
  ConsumerManager(RMAPHandler* handler) {
    ...
    writepointersemaphore =
        new SemaphoreRegister(this->rmaphandler, ConsumerManager::AddressOf_Writepointer_Semaphore_Register);
    initialize();
  }
 ...
 public:
  ...
  virtual uint32_t getWritePointer() {
    ...
    if (Debug::consumermanager()) { cout << "ConsumerManager::getWritePointer()..."; }
    // semaphore request
    ...
    writepointer =
        readdata[1] * 0x01000000 + readdata[0] * 0x00010000 + readdata[3] * 0x00000100 + readdata[2] * 0x00000001;
 ...
 public:
   ...
   */
  virtual uint32_t getReadPointer() { return readpointer; }
 ...
 public:
  ...
  virtual uint32_t setReadPointer(uint32_t readpointer) {
    ...
    if (Debug::consumermanager()) {
      ...
      readpointer_tmp =
          readdata[1] * 0x01000000 + readdata[0] * 0x00010000 + readdata[3] * 0x0100 + readdata[2] * 0x0001;
    }
 ...
 public:
  ...
  virtual uint32_t getGuardBit() {
    using namespace std;
    if (Debug::consumermanager()) { cout << "ConsumerManager::getGuardBit()..."; }
    std::vector<uint8_t> readdata;
    readdata.resize(2, 0);
    rmaphandler->read(adcboxRMAPNode, AddressOf_GuardBitRegister, 2, &readdata[0]);
    uint32_t guardbit = readdata[0];

    if (Debug::consumermanager()) { cout << "guard bit=" << guardbit << endl; }
    return guardbit;
  }
 ...
 public:
  ...
  virtual std::vector<uint8_t> read(uint32_t maximumsize = 4000) {
    ...
    if (readpointer == writepointer) {
      if (Debug::ringbuffer()) { cout << "ConsumerManager::read() writepointer is updated" << endl; }
      writepointer = getWritePointer();
      if (Debug::ringbuffer()) { cout << "ConsumerManager::read() guardbit is updated" << endl; }
    ...
    if (writepointer <= nextreadfrom && guardbit == 0) {
      ...
      if (Debug::ringbuffer()) { cout << "read mode : (writepointer>=nextreadfrom)" << endl; }
      ...
      try {
        ...
        //				cerr << hex << (uint32_t)addressFrom << " " << (uint32_t)(addressTo - addressFrom + 2)
        //<< endl;
      ...
      if (Debug::ringbuffer()) { cout << "read mode : (writepointer<nextreadfrom && guardbit==1)" << endl; }
      addressFrom = (nextreadfrom);
      ...
      //			returneddata = *rmaphandler->read(adcboxRMAPNode, addressFrom, (addressTo - addressFrom
      //+ 2));
      try {
        //				cout << "rmaphandler->read()ing." << endl;
        returneddata.resize((addressTo - addressFrom + 2), 0);
        rmaphandler->read(adcboxRMAPNode, addressFrom, (addressTo - addressFrom + 2), &returneddata[0]);
      } catch (...) { cout << "caught error when rmaphandler->read()ing." << endl; }
      nextreadfrom = increment_address(readpointer);
    }
    // read data stored in ring buffer
    if (Debug::ringbuffer()) {
 ...
 private:
  virtual uint32_t increment_address(uint32_t address) {
    ...
    if (Debug::channelmanager()) { cout << "done(" << hex << setw(8) << setfill('0') << address << dec << ")" << endl; }
    return address;
  }
};
