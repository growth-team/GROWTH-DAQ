...
class DataRecorder : public CxxUtilities::StoppableThread {
 ...
 public:
   ...
   */
  DataRecorder() : CxxUtilities::StoppableThread() {
  ...
  void setADCBox(SpaceWireADCBox* adcbox) {
    ...
    if (consumermanager != NULL) { delete consumermanager; }
  ...
  void saveHistograms(std::string filename) {
    ...
    treemutex.lock();
    TFile file(filename.c_str(), "recreate");
    for (size_t i = 0; i < SpaceWireADCBox::NumberOfChannels; i++) { histograms[i]->Write(); }
    treemutex.unlock();
  }
 ...
 public:
  /** Starts readout.
   */
  void resume() { resumed = false; }

  /** Stops read out.
   */
  void stop() { resumed = true; }
  ...
  void drawHistograms() {
    ...
    if (Debug::datarecorder()) { cout << "DataRecorder::drawHistograms()" << endl; }
    drawmutex.unlock();
  }
  ...
  void run() {
    ...
    while (!stopped) {
      ...
      if (!this->adcbox->getRMAPHandler()->isConnectedToSpWGbE()) { continue; }
      ...
      if (Debug::datarecorder()) { cout << "DataRecorder::run() read " << size << " bytes" << endl; }
      if (size == 0) { sleep(500); }
      readdata.resize(size_half);
      for (size_t i = 0; i < size_half; i++) { readdata[i] = tmp[2 * i + 1] * 0x100 + tmp[2 * i]; }
      treemutex.lock();
      for (size_t i = 0; i < size_half; i++) {
        switch (state) {
          case state_flag_fff0:
            pha_list_size = 0;
            if (readdata[i] == 0xfff0) { state = state_chid; }
            break;
          case state_chid:
            chid  = readdata[i];
