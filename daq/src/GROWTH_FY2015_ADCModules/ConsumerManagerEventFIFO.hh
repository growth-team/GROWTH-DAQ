...
public:
	...
	public:
		ConsumerManagerEventFIFODumpThread(ConsumerManagerEventFIFO* parent) {
			this->parent = parent;
		}
	public:
		void run() {
...
private:
	size_t readEventFIFO(uint8_t* buffer, size_t length) throw (RMAPInitiatorException) {
		...
		if (readSize != 0) {
			rmapHandler->read(adcRMAPTargetNode, InitialAddressOf_EventFIFO, (uint32_t) readSize, buffer);
		}
		return readSize;
	}
