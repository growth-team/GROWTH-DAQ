...
class ChannelManager {
 ...
 public:
  ...
  void startAcquisition(std::vector<bool> channelsToBeStarted) {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::startAcquisition()..."; }
    ...
    for (uint32_t i = 0; i < channelsToBeStarted.size(); i++) {
      ...
      if (channelsToBeStarted.at(i) == true) { writeData[1] = writeData[1] + tmp; }
      tmp = tmp * 2;
    }
    ...
    if (Debug::channelmanager()) { cout << "done" << endl; }
  }
 ...
 public:
  ...
  bool isAcquisitionCompleted() {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::isAcquisitionCompleted()..."; }
 ...
 public:
  ...
  bool isAcquisitionCompleted(size_t chNumber) {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::isAcquisitionCompleted(" << chNumber << ")..."; }
    std::vector<uint8_t> readData(2);
    rmapHandler->read(adcRMAPTargetNode, ChannelManager::AddressOf_StartStopRegister, 2, &readData[0]);
    if (Debug::channelmanager()) { cout << "done" << endl; }
 ...
 public:
  ...
  void stopAcquisition() {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::stopAcquisition()..."; }
    vector<uint8_t> writeData = {0x00, 0x00};
    startStopSemaphore->request();
 ...
 public:
  ...
  void setPresetMode(SpaceFibreADC::PresetMode presetMode) {
    ...
    if (Debug::channelmanager()) { cout << "ChannelManager::setPresetMode()..."; }
 ...
 public:
  ...
  void setPresetLivetime(uint32_t livetimeIn10msUnit) {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::setLivetime(" << livetimeIn10msUnit << ")..."; }
 ...
 public:
  ...
  double getRealtime() {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::getRealtime()..."; }
 ...
 public:
  ...
  void reset() {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::reset()..."; }
 ...
 public:
  ...
  void setPresetnEvents(uint32_t nEvents) {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::setPresetnEvents(" << nEvents << ")..."; }
    vector<uint8_t> writeData = {
        //
 ...
 public:
  ...
  void setadcClockCounter(uint32_t adcClockCounter) {
    using namespace std;
    if (Debug::channelmanager()) { cout << "ChannelManager::setadcClockCounter(" << adcClockCounter << ")..."; }
    vector<uint8_t> writeData = {
        //
