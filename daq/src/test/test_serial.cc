	...
	void run() {
		...
		while (!stopped) {
			...
			for (size_t i = 0; i < length; i++) {
				cout << "Received" << dec << i << ": " << "0x" << hex << right << setw(2) << setfill('0')
						<< (uint32_t) receiveBuffer[i] << endl;
			}
		}
	}
};
...
int main(int argc, char *argv[]) {
	...
	std::vector<uint8_t> sendData = {
			...
			0x00, 0x00, 0x00, 0x10,
			0xfe, 0x01, 0x4c, 0x00, 0xfe, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x10, 0x12, 0x00, 0x00, 0x02, 0xbe
	};
