...
class GROWTH_FY2015_ADC {
 ...
 public:
   ...
   */
  RMAPHandler* getRMAPHandler() { return rmapHandler; }
 ...
 public:
   ...
   */
  void freeEvent(GROWTH_FY2015_ADC_Type::Event* event) { eventDecoder->freeEvent(event); }
 ...
 public:
   ...
   */
  bool isAcquisitionCompleted() { return channelManager->isAcquisitionCompleted(); }

 public:
  /** Checks if data acquisition of single channel is completed.
   * 	return true if data acquisition is stopped.
   */
  bool isAcquisitionCompleted(size_t chNumber) { return channelManager->isAcquisitionCompleted(chNumber); }

 public:
  /** Stops data acquisition regardless of the preset mode of
   * the acquisition.
   */
  void stopAcquisition() { channelManager->stopAcquisition(); }
 ...
 public:
   ...
   */
  void setPresetMode(SpaceFibreADC::PresetMode presetMode) { channelManager->setPresetMode(presetMode); }

 public:
  /** Sets Livetime preset value.
   * @param livetimeIn10msUnit live time to be set (in a unit of 10ms)
   */
  void setPresetLivetime(uint32_t livetimeIn10msUnit) { channelManager->setPresetLivetime(livetimeIn10msUnit); }

 public:
  /** Sets PresetnEventsRegister.
   * @param nEvents number of event to be taken
   */
  void setPresetnEvents(uint32_t nEvents) { channelManager->setPresetnEvents(nEvents); }

 public:
  /** Get Realtime which is elapsed time since the board power was turned on.
   * @return elapsed real time in 10ms unit
   */
  double getRealtime() { return channelManager->getRealtime(); }
 ...
 public:
   ...
   */
  EventDecoder* getEventDecoder() { return eventDecoder; }
